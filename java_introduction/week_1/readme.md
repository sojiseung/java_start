...
- 조건식을 확인한다. 만약 참이면 코드 블럭을 실행하고, 거짓이면 wile문을 벗어난다.
- 조건식이 참이면 코드 블럭을 실행한다. 이후에 코드 블럭이 끝나면 다시 조건식 검사로 돌아가서 조건식을 검사하낟.(무한 반복)
...
do-while문은 코드를 먼저 실행하고 조건식을 확인한다. 
...
* break, continue
 ...
 -  코드블럭안에 break를 만나면 그 아래 코드는 실행이 안되고 반복문은 종료된다.
 -  코드블럭안에 continue를 만나면 그 아래 코드로 안넘어가고 다시 조건식으로 넘어간다 조건식이 거짓이 되면 조건식을 나온다.
...
3. 코드를 실행한다.
4. 코드가 종료되면 증감식을 실행한다. 주로 초기식에 넣은 반복 횟수와 관련된 변수의 값을 증가랗 때 사용한다.
5. 다시 2. 조건식 부터 시작한다. (무한반복)
     
* for문은 복잡해 보이지만 while문을 조금 더 편하게 다를 수 있도록 구조화 한 것 뿐이다.
* for문은 초기식, 증감식을 잡을때 여러가지를 잡을 수 있다. 예) for (int x = 1, y = 2; x <= 10; x++, y++) {...}
